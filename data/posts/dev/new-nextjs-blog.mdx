...
- env 根據留言系統加入環境變數資料
- google analytics 除了 siteMetadata 設定外，還可能需要修改 `next.config.js` 將網域加入白名單
...
- **TechNote：** 如首頁提到的，Navbar 有個 `TechNote` 分類。因為學習習慣和不想把部落格放一堆瑣碎的筆記，使用 `docusaurus` 另外架設程式筆記站
