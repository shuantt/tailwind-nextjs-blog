...
- 使用 react + tailwind 比 hexo 更好修改。相反對不使用這兩個技術的人來說是缺點，但寫 ejs 和 sass 使我心累
- 比 hexo 模板更有現代感！**就真的真的很好看**！(個人喜好XD)

## 遇到的問題

幾乎是開箱後代入個人設定後就可使用，但還是花幾天熟悉專案結構和做個人化設定，尤其是卡在調整 `contentlayer` 的部分，具體遇到以下問題：
...
其實可以專案 clone 下來後就直接部署，但我還是有做一些基本設定：
...
- **載入留言：** 作者僅放了 `Load Comments` 的按鈕，但樣式太不想可點擊的按鈕，因此調整了該按鈕樣式
...
雖然還是花了一點時間，但研究套件和看別人如何建構 next.js 專案收穫良多。總結就是這真的是個很棒的部落格模板，快用起來吧！如果架設時遇到問題也可以留言或聯絡我，我會盡力幫忙。
